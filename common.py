...
arm_vert = "Assets/Shared/shaders/pbr_shader_v_arm.vert"
arm_frag = "Assets/Shared/shaders/pbr_shader_f_arm.frag"
...
arm_shader = Shader.load(Shader.SL_GLSL, arm_vert, arm_frag)

arm_shader_attrib = ShaderAttrib.make(arm_shader)
arm_shader_attrib = arm_shader_attrib.setFlag(ShaderAttrib.F_hardware_skinning, True)
...
def start_particles(target_particle, in_model, inter_duration=1, inter_list=[]):
    base.enable_particles()
    particle_effect = ParticleEffect()
    particle_effect.load_config(target_particle)
    particle_effect.set_shader_off()
    particle_interval = ParticleInterval(particle_effect, in_model, 0, duration=inter_duration, softStopT=inter_duration/1.5)
    ...
    base.particle_seq.append(Func(effect_cleanup))
...
def preload_models(model_paths, callback=None):
    async def load_models():
        for path in model_paths:
            model = await base.loader.load_model(path, blocking=False)
            pieces = path.split("/")
            section_id = pieces[1]
            filename = pieces[-1]
