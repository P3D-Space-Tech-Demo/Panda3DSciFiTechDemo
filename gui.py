...
class GUI:
    ...
    def __init__(self, sections, shipSpecs, quit):
        properties = WindowProperties()
        properties.size = (1280, 720)
        base.win.request_properties(properties)

        base.win.set_clear_color(Vec4(0, 0, 0, 1))

        base.accept("window-event", self.windowUpdated)

        ### Main Menu

        self.mainMenuBackdrop = DirectFrame(
            frameSize=(-1/base.aspect2d.get_sx(), 1/base.aspect2d.get_sx(), -1, 1)
        )
        ...
        DirectLabel(
            text="INVASION OF THE MECHANOIDS!",
            parent=self.mainMenuBackdrop,
            scale=0.1,
            pos=(0, 0, 0.7625),
            text_align=TextNode.ALeft
        ...
        btn = self.makeButton("New Game", self.startGame, self.mainMenuPanel, 10)
        buttons.append(btn)

        btn = self.makeButton("Chapter Selection", self.selectSection, self.mainMenuPanel, 10)
        buttons.append(btn)

        btn = self.makeButton("Options", self.openOptions, self.mainMenuPanel, 10)
        buttons.append(btn)

        btn = self.makeButton("Quit", quit, self.mainMenuPanel, 10)
        buttons.append(btn)

        buttonSpacing = 0.2
        buttonY = (len(buttons) - 1) * 0.5 * buttonSpacing
        for btn in buttons:
            btn.set_pos(0.1, 0, buttonY)
        ...
        btn = self.makeButton("Chapter 1 // A Warrior's Choice", self.startSection, self.sectionMenu, 15, extraArgs = [0])
        buttons.append(btn)

        btn = self.makeButton("Chapter 2 // Across the Night", self.startSection, self.sectionMenu, 15, extraArgs = [1])
        buttons.append(btn)

        btn = self.makeButton("Chapter 3 // Facing the Foe", self.startSection, self.sectionMenu, 15, extraArgs = [2])
        buttons.append(btn)

        btn = self.makeButton("Chapter 4 // The Escape", self.startSection, self.sectionMenu, 15, extraArgs = [3])
        buttons.append(btn)
        ...
        btn.set_transparency(TransparencyAttrib.M_alpha)
        ...
        btn = self.makeButton("Light fighter", self.sectionSpecificMenuDone, self.shipSelectionMenu, 15,
                              extraArgs = [self.shipSelectionMenu, 1, shipSpecs[0]])
        buttons.append(btn)

        btn = self.makeButton("Medium Interceptor", self.sectionSpecificMenuDone, self.shipSelectionMenu, 15,
                              extraArgs = [self.shipSelectionMenu, 1, shipSpecs[1]])
        buttons.append(btn)

        btn = self.makeButton("Heavy Bombardment Platform", self.sectionSpecificMenuDone, self.shipSelectionMenu, 15,
        ...
        self.sections = [
            (sections[0], None),
            (sections[1], self.shipSelectionMenu),
            (sections[2], None),
            (sections[3], None),
        ]
        ...
        base.accept("f", self.toggleFrameRateMeter)
        self.showFrameRateMeter = False

    def toggleFrameRateMeter(self):
        self.showFrameRateMeter = not self.showFrameRateMeter

        base.set_frame_rate_meter(self.showFrameRateMeter)

    def windowUpdated(self, window):
        base.windowEvent(window)
        self.mainMenuBackdrop["frameSize"] = (-1/base.aspect2d.get_sx(), 1/base.aspect2d.get_sx(),
                                              -1/base.aspect2d.get_sz(), 1/base.aspect2d.get_sz())
        self.mainMenuPanel.set_x(base.render2d, -1)
        self.mainMenuPanel["frameSize"] = (0, 1.25, -1/base.aspect2d.get_sz(), 1/base.aspect2d.get_sz())
        self.sectionMenu.set_pos(base.render, -0.8, 0, 0)
        self.shipSelectionMenu.set_pos(base.render, -0.6, 0, 0)
    ...
    def startSectionInternal(self, index, data):
        ...
        self.currentSectionObject = sectionModule.start(self, data)
